...
function FIFO:getRestsFIFO(client_code, sec_code, class_code, comment, isShort)
    ...
    if isShort == 1 then
      sql = string.gsub (sql, 'fifo_long_2', 'fifo_short_2')
    end
...
function FIFO:decrease_short(trade)
      ...
      while rest_count <= table.maxn(restShort) and qty > 0 do
        ...
        if restShort[rest_count]['qty'] ~= 0 then
          ...
          local sql='INSERT INTO fifo_short_2 '..
...
function FIFO:increase_long(trade, qty)
	...
	local comment = self:get_deal_comment(trade.brokerref)
     ...
     local sql='INSERT INTO fifo_long_2 '..
...
--(лот нужен больше для информации, причем для спота)  
	...
	self:QueryTextOpenFifoPositionsLong_ver2(sec_code, class_code, account, isDetails) .. 
	' UNION ALL ' .. 
	self:QueryTextOpenFifoPositionsShort_ver2(sec_code, class_code, account, isDetails) ..
	' ) AS subq ' ..
...
function FIFO:QueryTextOpenFifoPositionsLong_ver2(sec_code, class_code, account, isDetails)
...
function FIFO:QueryTextOpenFifoPositionsShort_ver2(sec_code, class_code, account, isDetails)
                    ...
                    FROM
                              (
                                        SELECT
                                                ...
                                                ,
                                                   --ресурсы
                                                  -1            *SUM(res_qty)   AS qty
                                                , -1            *SUM(res_value) AS value
												, SUM(attr_exchange_comission)              AS commiss
												--bugfix. если в эти поля запишем не null, а например ноль, или пустую строку,
												--то потом получаем не совсем то, что надо.
												--если будет пустая строка в дате/времени, то в таблице робота в этих полях 
...
FROM
          fifo_long_2 AS ClosedPos
          LEFT JOIN
                    fifo_long_2 AS OpenPos
          ON
...
SELECT
        ...
        , securities.lotsize AS lot
FROM
          fifo_short_2 AS ClosedPos
          LEFT JOIN
                    fifo_short_2 AS OpenPos
          ON
                    OpenPos.dim_trade_num = ClosedPos.dim_trade_num
                    AND OpenPos.res_qty   < 0
          LEFT JOIN
